    ...
    <div className="auth-page">
      <div className="auth-card">
        ...
        <form className="auth-form" onSubmit={handleSubmit} noValidate>
          {globalError && (
            <div className="auth-error-banner"><ErrorIcon /> {globalError}</div>
          )}
          ...
          <div className="auth-field">
            ...
            <div className="auth-input-wrap">
              <input id="username" name="username" type="text" autoComplete="username"
                value={form.username} onChange={handleChange} placeholder="ton_pseudo"
                className={`auth-input ${errors.username ? 'error' : ''}`} />
              <span className="auth-input-icon"><UserIcon /></span>
            </div>
            {errors.username && <span className="auth-field-error"><ErrorIcon size={12} /> {errors.username}</span>}
          </div>
          ...
          <div className="auth-field">
            ...
            <div className="auth-input-wrap">
              <input id="email" name="email" type="email" autoComplete="email"
                value={form.email} onChange={handleChange} placeholder="ton@email.com"
                className={`auth-input ${errors.email ? 'error' : ''}`} />
              <span className="auth-input-icon"><MailIcon /></span>
            </div>
            {errors.email && <span className="auth-field-error"><ErrorIcon size={12} /> {errors.email}</span>}
          </div>
          ...
          <div className="auth-field">
            ...
            <div className="auth-input-wrap">
              <input id="password" name="password" type={showPassword ? 'text' : 'password'}
                ...
                placeholder="••••••••" className={`auth-input ${errors.password ? 'error' : ''}`} />
            ...
            {errors.password && <span className="auth-field-error"><ErrorIcon size={12} /> {errors.password}</span>}
            {form.password && (
              <>
                <div className="password-strength">
                  {[1, 2, 3].map(i => (
                    <div key={i} className={`strength-bar ${i <= strength ? STRENGTH_CLASSES[strength] : ''}`} />
                  ))}
                </div>
          ...
          <div className="auth-field">
            ...
            <div className="auth-input-wrap">
              <input id="confirm" name="confirm" type={showConfirm ? 'text' : 'password'}
                ...
                placeholder="••••••••" className={`auth-input ${errors.confirm ? 'error' : ''}`} />
              <span className="auth-input-icon"><LockIcon /></span>
              <button type="button" className="auth-eye-btn"
            ...
            {errors.confirm && <span className="auth-field-error"><ErrorIcon size={12} /> {errors.confirm}</span>}
          </div>
...
function MailIcon()  { return <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="4" width="20" height="16" rx="2"/><polyline points="2,4 12,13 22,4"/></svg>; }
...
function ErrorIcon({ size = 14 }) { return <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg>; }
